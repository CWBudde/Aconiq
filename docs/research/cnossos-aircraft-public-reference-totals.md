# CNOSSOS Aircraft Public Reference Totals

Status date: 2026-03-08

## Purpose

This note closes the narrow Phase 13 need for at least one license-safe,
attributable public aircraft benchmark source.
...
the shipped `cnossos-aircraft` baseline.
...
- public proof that the cited Irish airport outputs belong to the official END
...
- It is the official current public aircraft-noise action plan for Dublin
  Airport.
- The plan states that the strategic noise maps use 2021 as the operative year,
  with 2023 supplementary exposure data to better represent the post-COVID
  operating pattern.
- The plan states that the priority exposure checks are:
...
- It publishes explicit public aircraft exposure counts for the same regulatory
  framework around Dublin Airport.
- It reports 2023 exposed-population counts for:
  - above `65 dB Lden`
  - above `55 dB Lnight`
...
- people exposed above `65 dB Lden` in 2023:
  - `323`
- change against 2019 for `65 dB Lden`:
  - `+38`
- people exposed above `55 dB Lnight` in 2023:
  - `4,465`
- change against 2019 for `55 dB Lnight`:
  - `+2,932`
...
- a claim that the repo has at least one attributable, license-safe external
  reference source for CNOSSOS aircraft outputs
...
- enough public detail to claim full module-level CNOSSOS aircraft conformance

## Conclusion

For Phase 13, this is sufficient to treat the public-evidence requirement as
...
conformance or public-fixture milestone, not as an unfinished Phase 13 blocker.
