...
- Those deltas are computed against **one** grid map, not against every `RRLK*` the bundle holds. A project carries the same site as several grid maps, and two signals tell them apart: the geometry file list in the run's `.res` (does it contain `GeoWand.geo`?) and the `GNM<spacing>:<height>` token in its `RunCommands`, matched against the project's `RLKHEIGHT`. Both are carried onto `GridMapMetadata` at import time, as `geometry_files` and `run_layout`. The selected run, its candidates and the grounds for the choice are reported as `soundplan_raster_run`, `soundplan_raster_run_candidates` and `soundplan_raster_run_selection`; `soundplan_runs` stays the full discovered list. `--soundplan-grid-run` overrides the choice. The synthetic receivers are placed at the **selected run's** `run_layout.height_m`, falling back to `RLKHEIGHT` only when the run declared no layout, so an explicitly selected run is compared at the height it was computed at; a disagreement between the two is warned about naming both. When every candidate declared its geometry and none of them agrees with the model, the height is not consulted at all — the choice is reported as `geometry_contradicted` with a warning, because a height match there would present a provably different scenario as a confident answer.
