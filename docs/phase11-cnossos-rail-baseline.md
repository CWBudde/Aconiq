...
> 11 delivery as it was written on the status date above, and is kept
> unchanged as history. **It is not the current scope statement.**
> `cnossos-rail` carries the evidence tier `scaffold`: it holds no CNOSSOS-EU
> coefficient, no roughness or contact-filter spectra, no octave bands, and no
> propagation geometry; its base emission levels are invented. The binding
> description of what this module does and does not contain is
...
> **Superseded — read
> [`conformance/cnossos-umfangserklaerung.md`](conformance/cnossos-umfangserklaerung.md)
> instead.** Calling the module a "CNOSSOS rail preview implementation"
