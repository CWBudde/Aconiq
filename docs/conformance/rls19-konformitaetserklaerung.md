...
| C11 | §3.5.2 Gl. 14, Bodendämpfung              | `h_m`, the mean height of the propagation path above the ground, was computed as the mean path elevation minus an average terrain elevation that `computeTerrainAvgZ` returned as **0** whenever no declared slope edge crossed the path. The CLI never populates `cfg.Terrain`, so that was every path of every run that did not declare a Böschungskante. On a project carrying elevations `h_m` therefore became the path's height above sea level: at a site 400 m up with `s_gr = 200 m` the correct `h_m ≈ 2.25` gives `D_gr ≈ 4.4 dB`, while `h_m ≈ 402` gives −69 dB, which Gl. 14's `minimum 0` clamps to zero. The Bodendämpfung disappeared silently. The same zero reached a scene whose declared edges simply missed a given path, and the mirrored-path approximation carried the same datum error. | The ground under a path is resolved in order of preference and is never zero: declared `TerrainProfile` breakpoints, else the imported DTM sampled along the path (`geo/terrain.MeanRiseAboveChord`), else the straight line between the ground elevations at the two ends. The last case is the core correction — where no terrain is known the ground under each end is the ground at that end — and it is bit-identical to the previous output when those elevations are 0, which is why every flat-ground fixture is unmoved. Previous error: up to +4.8 dB, the whole of `D_gr`; measured at +4.39 dB on a 400 m site through the full CLI. The `i8_ascending` and `i9_receding` snapshots drop 0.5732 dB. |
...
- **No terrain-edge shielding and no terrain-following ground term on a mirrored
  leg.** The mean height of a reflected path is taken over the ground its two
  ends stand on, `(h_Quelle + h_Immissionsort)/2`, without following the ground
  profile between them, and terrain edges are not applied to it, because the
  image path does not lie over the sampled ground profile. Both conventions are
  the same one, applied consistently.
  This clause used to quote the formula as `(z_Quelle + z_Immissionsort)/2`, in
  absolute elevations. That was the C11 datum defect rather than a convention:
  the two agree only where the ground sits at Z = 0. The deviation itself — no
  terrain-following term, no terrain edges on a mirrored leg — is unchanged.
