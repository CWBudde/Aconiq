...
| R-009 | SoundPLAN interoperability is challenged on IP grounds, or shipped vendor catalogues are copied  | Low        | High   | Keep the Art. 6 record current in go-absolute-database; read only user-written files, never vendor-shipped datasets; no vendor code or expression | TBD   | Open   |
