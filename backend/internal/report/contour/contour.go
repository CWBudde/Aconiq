// Package contour turns a computed raster into ISO-band contour lines.
//
// It is its own package because marching squares has three consumers that must
// not be able to disagree: `aconiq export --format contour-geojson|contour-gpkg`,
// which writes the files a reviewer opens in QGIS; the WebAssembly kernel,
// which gives browser mode contours at all; and GET /api/v1/runs/{id}/contours,
// which gives API mode the same. Where a 55 dB line falls is read as an
// assessment, so a second implementation in any of the three would be a second
// answer to that question with nothing holding the two together.
//
// It is deliberately below all three rather than inside one of them, for the
// reason internal/geo/crstransform gives for the projection — but here there is
// a second, harder constraint. It used to live in internal/report/export, and
...
//
// The writers stay in internal/report/export, which is where writing files
// belongs, and that package keeps aliases for everything it re-exports so no
// caller and no golden had to move when this package was split out.
package contour
