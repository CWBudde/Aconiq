...
func newValidateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runValidateCommand(cmd, inputPath, writeReport)
		},
	}
...
func runValidateCommand(cmd *cobra.Command, inputPath string, writeReport bool) error {
	...
	if writeReport {
		err = persistValidationReport(store, &proj, report)
		if err != nil {
			return err
		}
	}
	...
	if state.Config.JSONLogs {
		...
		writeErr := writeCommandOutput(cmd.OutOrStdout(), true, jsonPayload)
		if writeErr != nil {
			return writeErr
		}

		return validationReportError(report)
	}
...
func persistValidationReport(store projectfs.Store, proj *project.Project, report modelgeojson.ValidationReport) error {
	reportPath := filepath.Join(store.Root(), ".noise", "model", "validation-report.json")
