...
func (cfg PropagationConfig) Validate() error {
	...
	if math.IsNaN(cfg.ReflectionGainDB) || math.IsInf(cfg.ReflectionGainDB, 0) || cfg.ReflectionGainDB < 0 {
		return fmt.Errorf("reflection_gain_db must be finite and >= 0")
	}
...
// ComputeReceiverPeriodLevels computes LrDay/LrNight at one receiver.
func ComputeReceiverPeriodLevels(receiver geo.Point2D, sources []RoadSource, cfg PropagationConfig) (PeriodLevels, error) {
	...
	for _, source := range sources {
		...
		dayContrib = append(dayContrib, emission.LrDay-a+cfg.ReflectionGainDB)
		nightContrib = append(nightContrib, emission.LrNight-a+cfg.ReflectionGainDB)
	}
