...
// PropagationConfig defines deterministic attenuation terms for BUB road.
...
func (cfg PropagationConfig) Validate() error {
	...
	if math.IsNaN(cfg.UrbanCanyonDB) || math.IsInf(cfg.UrbanCanyonDB, 0) || cfg.UrbanCanyonDB < 0 {
		return errors.New("urban_canyon_db must be finite and >= 0")
	}

	if math.IsNaN(cfg.IntersectionDensityPerKM) || math.IsInf(cfg.IntersectionDensityPerKM, 0) || cfg.IntersectionDensityPerKM < 0 {
		return errors.New("intersection_density_per_km must be finite and >= 0")
	}
...
func urbanCanyonEffect(cfg PropagationConfig) float64 {
	return cfg.UrbanCanyonDB
}

func intersectionEffect(cfg PropagationConfig) float64 {
	return math.Min(3.0, cfg.IntersectionDensityPerKM/20.0)
}
...
func attenuationTerms(distanceM float64, cfg PropagationConfig) propagationTerms {
	...
	return propagationTerms{
		...
		IntersectionDB: intersectionEffect(cfg),
	}
}

func totalAttenuation(terms propagationTerms) float64 {
	return terms.GeometricDB + terms.AirDB + terms.GroundDB - terms.UrbanCanyonDB - terms.IntersectionDB
}
...
func lineSourceLevelAtReceiver(emissionDB float64, receiver geo.Point2D, centerline []geo.Point2D, cfg PropagationConfig) float64 {
	...
	for i := range len(centerline) - 1 {
		for _, subsegment := range discretizeLineSegment(centerline[i], centerline[i+1]) {
