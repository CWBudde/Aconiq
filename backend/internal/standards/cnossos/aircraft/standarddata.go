...
// and not CNOSSOS-EU coefficients — Directive 2015/996 Annex II defines no
// aircraft method at all; see docs/conformance/cnossos-umfangserklaerung.md. A
...
func StandardData() framework.StandardData {
	return framework.StandardData{Tables: []framework.StandardDataTable{
		{Name: "preview-aircraft/model-version", Value: BuiltinModelVersion},
		{Name: "preview-aircraft/class-corrections", Value: aircraftClassCorrectionTable()},
		{Name: "preview-aircraft/operation-corrections", Value: operationCorrectionTable()},
		{Name: "preview-aircraft/procedure-corrections", Value: procedureCorrectionTable()},
		{Name: "preview-aircraft/thrust-mode-corrections", Value: thrustModeCorrectionTable()},
		{Name: "preview-aircraft/operation-mode-adjustments", Value: operationModeAdjustmentTable()},
		{Name: "preview-aircraft/bank-angle-samples", Value: bankAngleSampleTable()},
		{Name: "preview-aircraft/lateral-offset-samples", Value: lateralOffsetSampleTable()},
		{Name: "preview-aircraft/propagation-defaults", Value: DefaultPropagationConfig()},
	}}
}
...
// lateral directivity carried in the propagation defaults. The sample offsets
// sit below, at and above the 225 m where the term reaches its 1.5 dB cap, so
// both the divisor and the cap are pinned.
