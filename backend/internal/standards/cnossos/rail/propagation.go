...
func bridgeEffect(source RailSource, cfg PropagationConfig) float64 {
	if source.OnBridge {
		return cfg.BridgeCorrectionDB
	}

	return 0
}

func curveEffect(source RailSource, cfg PropagationConfig) float64 {
	...
	return severity * cfg.CurveSquealDB
}

func attenuationTerms(source RailSource, distanceM float64, cfg PropagationConfig) propagationTerms {
	...
	return propagationTerms{
		...
		CurveDB:     curveEffect(source, cfg),
	}
}

func totalAttenuation(terms propagationTerms) float64 {
	return terms.GeometricDB + terms.AirDB + terms.GroundDB - terms.BridgeDB - terms.CurveDB
}
...
func subsegmentContribution(emissionDB float64, receiver geo.Point2D, source RailSource, subsegment lineSubsegment, cfg PropagationConfig) float64 {
	distance := geo.Distance(receiver, subsegment.Midpoint)
	terms := attenuationTerms(source, distance, cfg)

	return emissionDB + 10*math.Log10(subsegment.LengthM) - totalAttenuation(terms)
}

func lineSourceLevelAtReceiver(emissionDB float64, receiver geo.Point2D, source RailSource, cfg PropagationConfig) float64 {
	contribs := make([]float64, 0, len(source.TrackCenterline))
	...
	for i := range len(source.TrackCenterline) - 1 {
		...
		for _, subsegment := range discretizeLineSegment(a, b) {
			contribs = append(contribs, subsegmentContribution(emissionDB, receiver, source, subsegment, cfg))
		}
	}
