...
func TestLateralOffsetRaisesMappedLevel(t *testing.T) {
	...
	if offsetLevels.Lday <= baseLevels.Lday {
		t.Fatalf("expected mapping directivity context to increase Lday: base=%f offset=%f", baseLevels.Lday, offsetLevels.Lday)
	}
}
...
func TestBUFAircraftProvenanceMetadataIncludesExpandedKeyParameters(t *testing.T) {
	t.Parallel()
	...
	if metadata["compliance_boundary"] != "baseline-preview-expanded-buf-aircraft-contract" {
	...
	if metadata["key_parameter.aircraft_procedure_type"] != "continuous_descent" || metadata["key_parameter.lateral_offset_m"] != "180" {
		t.Fatalf("expected expanded BUF key parameters in metadata: %#v", metadata)
	}
}
