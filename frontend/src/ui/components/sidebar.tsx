...
>(
  (
        ...
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          ...
          >
            <SheetHeader className="sr-only">
              <SheetTitle>{m.label_sidebar()}</SheetTitle>
              <SheetDescription>
                {m.msg_sidebar_mobile_description()}
              </SheetDescription>
            </SheetHeader>
    ...
    // Use sticky instead of fixed so the sidebar stays in the flex flow and
    // never overlaps content. The old fixed+spacer pattern was unreliable.
    return (
      <div
        ref={ref}
        className={cn(
          "group peer hidden text-sidebar-foreground md:flex flex-col flex-shrink-0",
          "sticky top-0 self-start z-10 h-svh overflow-hidden",
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    ...
    >
      <PanelLeft />
      <span className="sr-only">{m.action_toggle_sidebar()}</span>
    </Button>
  );
});
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ref={ref}
      data-sidebar="rail"
      aria-label={m.action_toggle_sidebar()}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={m.action_toggle_sidebar()}
      className={cn(
